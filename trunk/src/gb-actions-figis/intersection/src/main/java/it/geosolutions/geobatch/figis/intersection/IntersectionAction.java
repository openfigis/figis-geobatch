...
//    private WFS_1_0_0_DataStore dataStore = null;
    ...
    /**********
     * download a zip file from geoserver, save it in the tmp directory, uncompress it and return the name of the containe shape file
     * @param layername the layer to download
     * @param tmpDir the folder where saving and uncompress the downloaded zip file
    ...
    private String downloadFromGeoserver(String layername, String tmpDir){
    	...
    	try {
	    	...
	    	//uncompress the downloaded file and return the name of the shape file
	       	return ZipStreamReader.getShapeFileFromURLbyZIP(urlCollection, tmpDir, name);
    	} catch(Exception e) {
	...
	public SimpleFeatureCollection intersection(Intersection intersection, String tmpDir) {
...
//    		}
   ...
   // 	LOGGER.trace(">>>>>>>>>>>>>>>>>>>>>>>>>>> trgCollection: intersectionProcess: >>>>>>"+this+"<<<>>>"+dif_beg+" <<<<<<<<<<");
     ...
     * @throws Exception 
     */
    public synchronized boolean executeIntersectionStatements(String host, Config config, boolean simulate, String tmpdir) {
    	...
    	try {
	    	...
	    	try {
				...
				LOGGER.error("Problems creating the ORACLE datastore instance, check the parameters",e1);   
				return false;
			}
	    	...
	    	for (Intersection intersection:intersections){
	        	...
	        	if (status==Status.TOCOMPUTE) { // if the intersection should be computed
	        		...
	        		resultInt = intersection(intersection, tmpdir); // compute the intersection between the layers
...
//	        		}
	        		...
	        		try {
	        			...
	        			LOGGER.error("Cannot identify the geometry type of the intersection result",e);
	        		}
	        		...
	        		if (resultInt!=null && geometryType.equals("MultiPolygon")) {
						...
						try {
							...
							dataStoreOracle.perform(resultInt,getName(srcLayer), getName(trgLayer), srcCode, trgCode,itemsPerPage);
							...
							LOGGER.info("updating intersection "+intersection);
			        		Request.updateIntersectionById(host, id, intersection);	
						}
	        		}
	        		else {
	        			...
	        			try {
							...
							LOGGER.error("Some problems occured deleting intersection from the database identified by "+srcLayer+","+trgLayer,e);
						}
	        		}
	        	}
	        }
    ...
    public Config basicChecks(){
        ...
        LOGGER.info("Config information correctly read. Trying to connect to Geoserver on "+config.getGlobal().getGeoserver().getGeoserverUrl());
            ...
            try {
                if ((ev = events.remove()) != null) {
                    ...
                    if (config!=null) {
                    	...
                    	LOGGER.info("Creating the temporary dir: "+tmpDirName);
                    	int randomIndex = generator.nextInt(100000000);
                    	File tmpDir = TmpDirManager.createTmpDir(tmpDirName+System.getProperty("file.separator")+Integer.toString(randomIndex));
                        ...
                        if (!areIntersectionsUpdated) LOGGER.error("Problems occurred during the execution, check the log to see the reason");
                        // delete the tmpDir after execution
                        LOGGER.trace("Deleting the temporary dir: "+tmpDirName);
                        TmpDirManager.deleteDir(tmpDir);
                        LOGGER.trace(tmpDirName+" successfully deleted");
                    } else LOGGER.error("Cannot read the configuration "+config+". Skip execution");
