...
public class GeomCompareServiceHandlerBean extends ComponentSupport { //  implements MessageExchangeListener {
    ...
    public void quartz(MessageExchange exchange) throws MessagingException, AccessException {
        if (exchange.getStatus() == ExchangeStatus.ACTIVE) {
            ...
            LOGGER.info("Quartz trigger at " + strDate);
    ...
    private void handle(MessageExchange exchange)
        ...
        try {
            ...
            } else if ("getPopulatedCombinations".equals(service)) {
                body = doGetPopulatedCombinations(exchange);                
    ...
    private String doAddCombination(MessageExchange exchange) throws MessagingException, AccessException, ParamException {
        ...
        boolean preserve_target = getOptionalBoolParam(in, "preserve_target");
        rebuilder.addCombination(source, target, mask, sourceCode, targetCode, preserve_target);
    ...
    private String doGetPopulatedCombinations(MessageExchange exchange) throws MessagingException, AccessException, ParamException {
        NormalizedMessage in = exchange.getMessage("in");
        String jsonArray = JsonSerializer.serialize( rebuilder.getPopulatedCombinations() );  
        
        return "{'status':'DONE',  'service':'getPopulatedCombinations', 'combinations':"+ jsonArray+"}";
    }
