...
public class DataStoreManager {
    ...
    /**
     * Accepts params for connecting to a generic datastore
     */
    public DataStoreManager(Map<String, Serializable> dbMapConfiguration) throws Exception {
        ...
        try {
            ...
            try {
                ...
                throw new Exception(
                        "Error creating the data store, check the connection parameters", e);
            }

            transaction = new DefaultTransaction();

            SYNCH.acquire();
            initTables(dataStore, transaction);
    ...
    /*********
     * delete all the instances of srcLayer and trgLayer from the permanent tables copy the intersections and the stats temporary information into the
     * permanent tables
     * 
     * @param ds
     * @param tx
    ...
    private void action(Transaction tx, String srcLayer, String trgLayer, String srcCode,
        ...
        FeatureStore featureStoreTmpData = (FeatureStore) dataStore
                .getFeatureSource(STATS_TMP_TABLE);
        FeatureStore featureStoreTmpGeom = (FeatureStore) dataStore
                .getFeatureSource(SPATIAL_TMP_TABLE);

        featureStoreTmpData.setTransaction(tx);
        featureStoreTmpGeom.setTransaction(tx);

        FeatureStore featureStoreData = (FeatureStore) dataStore.getFeatureSource(STATS_TABLE);
        FeatureStore featureStoreGeom = (FeatureStore) dataStore.getFeatureSource(SPATIAL_TABLE);
    ...
    private void deleteOldInstancesFromPermanent(String srcLayer, String trgLayer,
            String srcLayerCode, String trgLayerCode, FeatureStore featureStoreData,
            FeatureStore featureStoreGeom) throws Exception {
        LOGGER.info("Deleting old instances of the intersection between " + srcLayer + " and "
                + trgLayer);
    ...
    private void cleanTempTables(Transaction tx, SimpleFeatureCollection collection,
            String srcLayer, String trgLayer, String srcCode, String trgCode, int itemsPerPage)
            throws IOException {
        LOGGER.trace("Cleaning temp tables");

        FeatureStore featureStoreData = (FeatureStore) dataStore
                .getFeatureSource(SPATIAL_TMP_TABLE);
        featureStoreData.setTransaction(tx);

        FeatureStore featureStoreGeom = (FeatureStore) dataStore.getFeatureSource(STATS_TMP_TABLE);
        featureStoreGeom.setTransaction(tx);
    ...
    private void saveToTemp(Transaction tx, SimpleFeatureCollection source, String srcLayer,
        ...
        LOGGER.info("Saving intersections between " + srcLayer + " and " + trgLayer
        ...
        try {
            ...
            featureStoreData = (FeatureStore) dataStore.getFeatureSource(STATS_TMP_TABLE);
            featureStoreData.setTransaction(tx);

            featureStoreGeom = (FeatureStore) dataStore.getFeatureSource(SPATIAL_TMP_TABLE);
            featureStoreGeom.setTransaction(tx);
            ...
            while (iterator.hasNext()) {
                ...
                String intersectionID = srcLayer + "_" + srcCode + "_" + trgLayer + "_" + trgCode
                        + i;
    ...
    /**
     * recall the steps from updating the database firstly delete all the information from temp tables then, perform the intersection, update
     * temporary and finally update permanent
     * 
     * @param collection
     * @param srcLayer
     * @param trgLayer
     ...
     * @throws IOException
     */
    public boolean saveAll(SimpleFeatureCollection collection, String srcLayer, String trgLayer,
            String srcCode, String trgCode, int itemsPerPage) throws Exception {

        boolean res = false;
        Transaction tx = null;
        try {
            tx = new DefaultTransaction();
            LOGGER.trace("Performing data saving: SRCLAYER " + srcLayer + ", SRCCODE " + srcCode
                    + ", TRGLAYER " + trgLayer + ", TRGLAYER " + trgCode);
     ...
     * @throws IOException
     */
    public void deleteAll(String srcLayer, String trgLayer, String srcCode, String trgCode)
            throws IOException {
        ...
        try {
            ...
            FeatureStore featureStoreData = (FeatureStore) dataStore.getFeatureSource(STATS_TABLE);
            FeatureStore featureStoreGeom = (FeatureStore) dataStore
                    .getFeatureSource(SPATIAL_TABLE);
